...
local dataPath = "/usr/share/wincraft/server/"
local binPath = "/usr/bin/wincraft/server/"
local configPath = "/etc/wincraft/"
...
local loadJsonData = function(fileName)
	...
	if fileName == nil then
		f = io.open(configPath.."settings_server.json", "r")
	else
		f = io.open(dataPath..fileName, "r")
	end
...
local saveJsonData = function(fileName, data)
	local f=io.open(dataPath..fileName,"w")
...
local updateVariableFromSave = function(varName)
	...
	if fs.exists(binPath.."variables/"..varName) then
		local f = io.open(binPath.."variables/"..varName, "r")
		...
		fs.remove(binPath.."variables/"..varName)
	end
end
...
local saveVariable = function(fileName, data)
	local f=io.open(binPath.."variables/"..fileName,"w")
	f:write(tostring(data))
	f:close()
end

local removeVariable = function(fileName)
	local fs = require "filesystem"; fs.remove(binPath.."variables/"..fileName)
end

local saveOrder = function(fileName, data)
	local f = io.open(binPath.."orders/"..fileName..".lua","w")
	f:write(data)
	f:close()
end

local removeOrder = function(fileName)
	local fs = require "filesystem"; fs.remove(binPath.."orders/"..fileName..".lua")
end
...
local uVar = function(eventType,dest,src,aport,strength,order, oldVarName, newVarName, actualVar)
	...
	if oldVarName ~= newVarName then removeVariable(oldVarName) end
