...
local dataPath = "/usr/share/wincraft/client/"
local binPath = "/usr/bin/wincraft/client/"
...
local readData = function()
	...
	dataApplis = json.decode(io.open(dataPath.."dataApplis.json", "r"):read("*all"))
	json = nil; package.loaded.json = nil
end

local saveData = function()
	local f=io.open(dataPath.."dataApplis.json","w")
	local json = require "json"
...
local saveAppliFile = function(data, name)
	local f=io.open(binPath.."applications/custom/"..name,"w")
	f:write(data)
	f:close()
end
...
deleteAppliFile = function(fileName)
	...
	local fs = require "filesystem"; fs.remove(binPath.."applications/custom/"..aFileName..".lua")
end
