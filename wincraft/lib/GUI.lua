...
local function roundedButtonDraw(button)
	...
	if backgroundColor then
		...
		if button.height > 1 then
			buffer.drawText(button.x + 1, button.y, backgroundColor, string.rep("_", button.width - 2))
...
function GUI.drawShadow(x, y, width, height, transparency, thin)
	if thin then
		...
		buffer.drawText(x + width, y, 0x0, "_", transparency)
	else
...
local function colorSelectorDraw(colorSelector)
	...
	if colorSelector.height > 1 and colorSelector.drawLine then
		buffer.drawText(colorSelector.x, colorSelector.y + colorSelector.height - 1, overlayColor, string.rep("_", colorSelector.width), 0.8)
	end
...
function GUI.comboBox(x, y, width, itemSize, backgroundColor, textColor, arrowBackgroundColor, arrowTextColor)
	...
	--comboBox.itemChanged = function(index) end--jpi
	comboBox.itemChanged = function() end
	...
	comboBox.dropDownMenu.onMenuClosed = function(index)
		...
		--comboBox.itemChanged(index)--jpi
		comboBox.itemChanged()--jpi
